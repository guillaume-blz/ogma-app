...
interface DataGridProps {
  ...
  onToggleOrder: (column: string) => void;
}

export function DataGrid({ result, orderBy, onToggleOrder }: DataGridProps) {
      ...
      <table className="min-w-full text-xs border-collapse">
        <thead className="sticky top-0 z-10 bg-muted/90 backdrop-blur-sm">
          <tr>
            ...
            {columns.map((col) => {
              const dir = orderBy?.column === col ? orderBy.direction : null;
              return (
                <th
                  key={col}
                  onClick={() => onToggleOrder(col)}
